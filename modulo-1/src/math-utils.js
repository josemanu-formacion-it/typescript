...
export function calcularMediana(datos: number[]): number | null {
  ...
  if (ordenado.length % 2 !== 0) {
    return ordenado[medio] as number;
  }
  return ((ordenado[medio - 1] as number) + (ordenado[medio] as number)) / 2;
}
...
export function filtrarAtipicos(datos: number[], limite: number = 1.5): number[] | null {
  ...
  const q1Index = Math.floor(ordenado.length * 0.25);
  const q3Index = Math.floor(ordenado.length * 0.75);
  
  const q1 = ordenado[q1Index] as number;
  const q3 = ordenado[q3Index] as number;
  const iqr = q3 - q1;
